...
if(isset($_GET['qty']) && ($_GET['qty'] == "minus")){
    $counter = $_GET['count'];
    $sql = "SELECT code, price, qty FROM cart WHERE email = '$email' AND count = '$counter' LIMIT 1";
    $resultMinus = mysqli_query($con, $sql);
    $rowMinus = mysqli_fetch_array($resultMinus, MYSQLI_ASSOC);
    $codeCoffee = $rowMinus['code'];
    $sqlCode = "SELECT price FROM coffees WHERE code = '$codeCoffee'";
    ...
    $sqlUpdate = "UPDATE cart SET price = '$newPrice', qty = '$quantity' WHERE email = '$email' AND count = '$counter'";
...
if(isset($_POST['addToCart'])){
    ...
    $checkDupQuery = "SELECT coffee, sugar, sugarType, milk, cinnamon, choco, qty FROM cart WHERE email = '$email' AND coffee = '$nameCheckDup' AND sugar = '$_POST[$sugarCheckDup]' AND sugarType = '$_POST[$sugarTypeCheckDup]' AND milk = '$milkCheck' AND cinnamon = '$cinnamonCheck' AND choco = '$chocoCheck'";    $resultCheckDup = mysqli_query($con, $checkDupQuery);
    if(mysqli_num_rows($resultCheckDup) == 1){
        ...
        foreach($coffees as $row){
            ...
            if (isset($_POST[$sugar])){
                $count++;
                $cart_query = "INSERT INTO cart (email, count, code, coffee, sugar, sugarType, price, qty) VALUES('$email', '$count', '$code', '$name', '$_POST[$sugar]', '$_POST[$sugarType]', '$price', 1)";
